package com.video.compres.video;
...
public class MediaController {
    public interface OnProgressUpdateListener {
        void onHandleVideoSize(double size);
    }
...
//    }
    ...
    @TargetApi(16)
    public boolean convertVideo(final String path, String outPath, OnProgressUpdateListener updateListener) {
        ...
        String bitrates = retriever.extractMetadata(MediaMetadataRetriever.METADATA_KEY_BITRATE);
        long startTime = -1;
        long endTime = -1;

        int resultWidth = 960;
        int resultHeight = 544;
        int rotationValue = Integer.valueOf(rotation);
        int originalWidth = Integer.valueOf(width);
        int originalHeight = Integer.valueOf(height);
        if ((rotationValue == 270.0 || rotationValue == 90) && originalWidth > 1280) {
            resultWidth = originalHeight / 2;
            resultHeight = originalWidth / 2;
        } else {
            resultWidth = originalHeight / 2;
            resultHeight = originalWidth / 2;
...
//            resultHeight = originalHeight / 2;
        ...
        int bitrate = Integer.valueOf(bitrates) / 6;
        int rotateRender = 0;
                                    ...
                                    if (index == videoIndex) {
                                        ...
                                        if (inputBufIndex >= 0) {
                                            ...
                                            if (updateListener != null)
                                                updateListener.onHandleVideoSize(FormetFileSize(totalsize, SIZETYPE_KB));
                                            Log.e("tmessage", "chunkSize:" + totalsize);
                                        }
